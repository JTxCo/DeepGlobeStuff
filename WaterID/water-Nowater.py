...
# List of incorrect segmentation maps
...
def label_water(mask: np.array) -> np.array:
    # Correctly identify and label water pixels
    water_mask = (
        (mask[:, :, 0] == 0) & 
        (mask[:, :, 1] == 0) & 
        (mask[:, :, 2] == 255)
...
with concurrent.futures.ThreadPoolExecutor() as executor:
    results = list(tqdm(executor.map(lambda row: process_sample(row[0], row[1]), metadata_df[['sat_image_path', 'mask_path']].values),
